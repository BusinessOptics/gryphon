...
def get_task_def_list():
    lst_raw = list_all_children(ecs.list_task_definitions, "taskDefinitionArns")
    ...
    for definition in t_definitions:
        ...
        temp_task_def = TaskDefinition(
            arn=arn, family=task_fam, revision=definition["revision"]
        )
        cont_defs = []
        for container in definition["containerDefinitions"]:
            environments = {
                env["name"]: env["value"] for env in container["environment"]
            }
            temp_cont_def = ContainerDefinition(
                name=container["name"],
    ...
    for fam in task_fam_list.keys():
        task_fams.append(
            TaskFamily(
                name=fam, task_defs=sorted(task_fam_list[fam], key=lambda x: x.revision)
            )
        )
...
class Cluster:
    def __init__(self, name):
        ...
        all_container_instances = list_all_children(
            ecs.list_container_instances, "containerInstanceArns", cluster=self.name
        )
        ...
        for task in task_info:
            ...
            tasks[task_arn] = Task(
                arn=task_arn, cluster=self, last_status=task["lastStatus"]
            )
            ...
            if "containerInstanceArn" in task:
                ...
                fargate_tasks.append(tasks[task_arn])

            conts = []

            for cont in task["containers"]:
                container_arn = cont["containerArn"]
                containers[container_arn] = Container(
                    arn=container_arn,
                    container=cont,
                    task=tasks[task_arn],
        ...
        for definition in t_definitions:
            ...
            task_defs[task_def_arn] = TaskDefinition(
                arn=task_def_arn,
            ...
            for cont_def in definition["containerDefinitions"]:
                ...
                environments = {
                    env["name"]: env["value"] for env in cont_def["environment"]
                }
                conts = [
                    cont
                    for cont in containers.values()
                    if cont.name == container_def_name
                ]
                temp_container = ContainerDefinition(
                    name=container_def_name,
        ...
        container_instances = ecs.describe_container_instances(
            cluster=self.name, containerInstances=list(cont_inst_arn.keys())
        )["containerInstances"]

        ec2_id_to_ci = {
            container["ec2InstanceId"]: container for container in container_instances
        }

        logging.info("Describe autoscaling instances")
        auto_instances = {
            auto_inst["InstanceId"]: auto_inst
            for auto_inst in auto_scaling.describe_auto_scaling_instances(
                InstanceIds=list(ec2_id_to_ci.keys())
        ...
        ec2_instances = {
            ...
            for inst in ec2.instances.filter(InstanceIds=list(ec2_id_to_ci.keys()))
        }
        ...
        self.fargate = Fargate(cluster=self, tasks=fargate_tasks)

        for task in fargate_tasks:
            task.instance = self.fargate
    ...
    def stop_task(self, task_arn):
        response = ecs.stop_task(
            cluster=self.name, task=task_arn, reason="Task stopped by user (gryphon)"
        )
...
class Task:
    ...
    @property
    def actions(self):
        if self.instance.is_fargate:
            return ["run", "environment"]
        else:
            return ["run", "exec", "attach", "environment"]


class Instance:
    def __init__(
        self, ec2_instance, container_instance, autoscaling_instance, cluster, tasks
    ):
...
class Fargate:
    def __init__(self, cluster, tasks):
        self.cluster = cluster
        self.tasks = tasks
        self.is_fargate = True


class Container:
    def __init__(
        self, arn=None, container=None, task=None, status=None, container_def=None
    ):
        self.arn = arn
...
class TaskDefinition:
    def __init__(
        self, arn=None, family=None, revision=None, tasks=None, container_defs=None
    ):
        self.arn = arn
        self.family = family
